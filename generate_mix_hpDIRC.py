...
def main(config,args):
    ...
    if config['method'] in ['Kaon','MixPK']:
        ...
        PID = 321
    if config['method'] == "Pion" or "MixPK":
    ...
    while running_gen < args.n_tracks:
        if config['method'] in ['Kaon','MixPK']:
            ...
            if len(generations) == args.n_dump:
                ...
                out_path_ = os.path.join(out_folder,str(config['method'])+f"_p_{args.momentum}_theta_{args.theta}_PID_{config['method']}_ntracks_{len(generations)}_{file_count}.pkl")
    ...
    if config['method'] == 'Kaon' or config['method'] == 'MixPK':
        print(" ")
    ...
    if config['method'] == 'Pion' or config['method'] == 'MixPK':
        print(" ")
...
if __name__=='__main__':
    ...
    parser.add_argument('-nd', '--n_dump', default=1e4, type=int, help='Number of particles to dump per .pkl file.')
