...
class hpDIRC_DLL_Dataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        #print(unscaled_conds)
        return hits,conds,PID,n_hits,unscaled_conds,LL_k,LL_pi



class hpDIRCCherenkovPhotons(Dataset):
    def __init__(self,kaon_path=None,pion_path=None,mode=None,combined=False,inference=False,stats={"x_max": 250,"x_min":2.0,"y_max":230.07142857142858,"y_min":2.0,"time_max":157.00,"time_min":0.0,"P_max":10.0 ,"P_min":0.5 ,"theta_max": 170.0,"theta_min": 11.0}):
        if mode is None:
            print("Please select one of the following modes:")
            print("1. Pion")
            print("2. Kaon")
        ...
        if mode == "Kaon":
            ...
            if not self.inference:
                print('Using training mode.')
                df = pd.read_feather(kaon_path)#,sep=',',index_col=None)
                df = df[(df.time < self.stats['time_max']) & (df.theta > self.stats['theta_min']) & (df.theta < self.stats['theta_max']) & (df.P > self.stats['P_min']) & (df.P < self.stats['P_max'])]#.to_numpy()
            ...
            if not self.inference:
                print('Using training mode.')
                df = pd.read_feather(pion_path)#,sep=',',index_col=None)
                print(df.shape)
                df = df[(df.time < self.stats['time_max']) & (df.theta > self.stats['theta_min']) & (df.theta < self.stats['theta_max']) & (df.P > self.stats['P_min']) & (df.P < self.stats['P_max']) & (df.phi == 0)]#.to_numpy()
                self.data = df[columns].to_numpy()
                ...
                print("Using inference mode.")
                df = pd.read_feather(pion_path)#,sep=',',index_col=None)
                df = df[(df.time < self.stats['time_max']) & (df.theta > self.stats['theta_min']) & (df.theta < self.stats['theta_max']) & (df.P > self.stats['P_min']) & (df.P < self.stats['P_max']) & (df.phi == 0.0)]#.to_numpy()
                self.data = df[columns].to_numpy()
