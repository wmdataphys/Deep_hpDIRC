...
for root_file in /sciclone/data10/jgiroux/Cherenkov/Real_Data/hd_root/*.root; do
    # Extract the filename without extension
    filename=$(basename "$root_file")
    filename_noext="${filename%.*}"

    # Construct the output JSON filename in the json folder
    json_file="/sciclone/data10/jgiroux/Cherenkov/Real_Data/json/${filename_noext}.json"
