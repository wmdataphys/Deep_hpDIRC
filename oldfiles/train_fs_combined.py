...
def main(config,resume):
    ...
    pion_hits,pion_conds = create_dataset(config['dataset']['pion_data_path'])
    kaon_hits,kaon_conds = create_dataset(config['dataset']['kaon_data_path'])
    hits = np.concatenate([pion_hits,kaon_hits],axis=0)
    pion_conds = np.concatenate([pion_conds,np.c_[np.ones_like(pion_conds[:,0])]],axis=1)
    kaon_conds = np.concatenate([kaon_conds,np.c_[np.zeros_like(kaon_conds[:,0])]],axis=1)
    conds = np.concatenate([pion_conds,kaon_conds],axis=0)
    ...
    train_hits = hits[:N_t]
    train_cond = conds[:N_t]

    val_hits = hits[N_t:]
    val_cond = conds[N_t:]

    train_dataset = TensorDataset(torch.tensor(train_hits),torch.tensor(train_cond))
    val_dataset = TensorDataset(torch.tensor(val_hits),torch.tensor(val_cond))
    ...
    net = create_nflows(train_hits.shape[1],train_cond.shape[1],8)
