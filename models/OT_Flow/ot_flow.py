...
class OT_Flow(nn.Module):
    ...
    def _sample(self,num_samples,context,nt=40):
        ...
        # x = self.unscale(samples[:,0].flatten(),x_mu,x_std)
        # y = self.unscale(samples[:,1].flatten(),y_mu,y_std)
        # t = self.unscale(samples[:,2].flatten(),time_mu,time_std)
    ...
    def create_tracks(self,num_samples,context,plotting=False):
        ...
        #x = updated_hits[:,0].detach().cpu()
        #y = updated_hits[:,1].detach().cpu()
