...
from models.OT_Flow.Phi import *
...
def OTFlowProblem(x, Phi, tspan=[0,1] , nt=6, stepper="rk4", alph =[1.0,1.0,1.0],conds=None,dist_func=None,training=True):
    """
    ...
    if stepper=='rk4':
        for k in range(nt):
            z = stepRK4(odefun, z, Phi, alph, tk, tk + h,conds)
            tk += h
    elif stepper=='rk1':
        for k in range(nt):
            z = stepRK1(odefun, z, Phi, alph, tk, tk + h,conds)
            tk += h

    # ASSUME all examples are equally weighted
    if not training:
        costC  = C(z,conds=conds,dist_func=dist_func)
        costL = z[:,-2]
        costR = z[:,-1]
    else:
        costC  = torch.mean(C(z,conds=conds,dist_func=dist_func))
...
def stepRK4(odefun, z, Phi, alph, t0, t1,conds):
    """
    ...
    K = h * odefun(z0, t0, Phi, alph=alph,conds=conds)
    z = z0 + (1.0/6.0) * K

    K = h * odefun( z0 + 0.5*K , t0+(h/2) , Phi, alph=alph,conds=conds)
    z += (2.0/6.0) * K

    K = h * odefun( z0 + 0.5*K , t0+(h/2) , Phi, alph=alph,conds=conds)
    z += (2.0/6.0) * K

    K = h * odefun( z0 + K , t0+h , Phi, alph=alph,conds=conds)
    z += (1.0/6.0) * K
...
def integrate(x, net, tspan , nt, stepper="rk4", alph =[1.0,1.0,1.0], intermediates=False,conds=None):
    """
    ...
    if intermediates: # save the intermediate values as well
        ...
        if stepper == 'rk4':
            for k in range(nt):
                zFull[:,:,k+1] = stepRK4(odefun, zFull[:,:,k] , net, alph, tk, tk+h,conds=conds)
                tk += h
        elif stepper == 'rk1':
            for k in range(nt):
        ...
        if stepper == 'rk4':
            for k in range(nt):
                z = stepRK4(odefun,z,net, alph,tk,tk+h,conds=conds)
                tk += h
        elif stepper == 'rk1':
            for k in range(nt):
...
def odefun(x, t, net, alph=[1.0,1.0,1.0],conds=None):
    ...
    if isinstance(t, torch.Tensor):
        z = pad(x[:, :d], (0, 1, 0, 0), value=t.item()) # concatenate with the time t
    else:
        z = pad(x[:, :d], (0, 1, 0, 0), value=t)

    gradPhi, trH = net.trHess(z,conds=conds)
