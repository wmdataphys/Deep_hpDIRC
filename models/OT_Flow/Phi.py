...
class ResNN(nn.Module):
    ...
    def forward(self, x, c=None):
        ...
        if c is None:
            ...
            x = self.act(self.layers[0].forward(torch.concat([x,c],dim=-1)))
...
class Phi(nn.Module):
    def __init__(self, nTh, m, d, r=10, alph=[1.0] * 5,conditional_dim=2):
        """
        ...
        self.A  = nn.Parameter(torch.zeros(r, d+conditional_dim+1) , requires_grad=True)
        self.A  = nn.init.xavier_uniform_(self.A)
        self.c  = nn.Linear( d+conditional_dim+1  , 1  , bias=True)  # b'*[x;t] + c
        self.w  = nn.Linear( m    , 1  , bias=False)

        self.N = ResNN(d, m, nTh=nTh,conditional_dim=conditional_dim)
    ...
    def trHess(self,x, justGrad=False ,conds=None):
        """
        ...
        if conds is None:
            ...
            opening     = N.layers[0].forward(torch.concat([x,conds],dim=-1)) # K_0 + S(;|y) + b_0
        ...
        if conds is None:
            ...
            grad = z[0] + torch.mm(symA, torch.concat([x,conds],dim=-1).t() ) + self.c.weight.t()

        #print(z[0].shape,symA.shape,grad.shape)
