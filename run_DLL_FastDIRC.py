...
def main(config,resume):
    ...
    for i,(p_as_p,p_as_k) in enumerate(zip(pion_as_pion,pion_as_kaon)):
        ...
        with torch.set_grad_enabled(False):
            support_pions = pion_net._sample(num_samples=1,context=c).squeeze(1).detach().cpu().numpy()
        pion_hyp_pion = DIRC_DLL.get_log_likelihood(support_pions,h.detach().cpu().numpy())
        ...
        with torch.set_grad_enabled(False):
            support_kaons = kaon_net._sample(num_samples=1,context=c).squeeze(1).detach().cpu().numpy()

        pion_hyp_kaon = DIRC_DLL.get_log_likelihood(support_kaons,h.detach().cpu().numpy())
    ...
    for i,(k_as_k,k_as_p) in enumerate(zip(kaon_as_kaon,kaon_as_pion)):
        ...
        kaon_hyp_kaon = DIRC_DLL.get_log_likelihood(support_kaons,h.detach().cpu().numpy())
        ...
        kaon_hyp_pion = DIRC_DLL.get_log_likelihood(support_pions,h.detach().cpu().numpy())
    ...
    with open("Pion_DIRC_DLL_Results.pkl","wb") as file:
        pickle.dump(LL_Pion,file)
    with open("Kaon_DIRC_DLL_Results.pkl","wb") as file:
